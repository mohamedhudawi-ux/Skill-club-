            ...
            <Card className="h-full">
              <div className="p-6">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-stone-900">{club.name}</h2>
                    <p className="text-sm text-emerald-600 font-medium">Active Club</p>
                  </div>
                </div>
                <p className="text-stone-600 mb-6 line-clamp-3">{club.description}</p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-4">
                    {members.filter(m => m.clubId === club.id).map(member => (
                      <div key={member.id} className="flex items-center gap-3 bg-stone-50 p-2 rounded-lg flex-1 min-w-[140px]">
                        {member.photoUrl ? (
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-stone-500 uppercase">{member.position}</p>
                        </div>
                      </div>
                    ))}
                  </div>
