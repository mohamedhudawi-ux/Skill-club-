          ...
          if (userDoc.exists()) {
            ...
            try {
              if (isMasterAdminEmail && data.role !== 'master_admin') {
                ...
                await setDoc(doc(db, 'users', firebaseUser.uid), finalProfile);
              } else if (isSkillEduEmail && data.role !== 'student' && !isAdminEmail && !isSafaEmail && !isStaffEmail && !isAcademicEmail) {
            ...
            if (registrationEnabled || isMasterAdminEmail || isAdminEmail || isSafaEmail || isStaffEmail || isPreRegisteredStudent || isSkillEduEmail || isAcademicEmail) {
              // Create a default profile
              const newRole: UserRole = isMasterAdminEmail ? 'master_admin' : (isAdminEmail ? 'admin' : (isSafaEmail ? 'safa' : (isAcademicEmail ? 'academic' : (isStaffEmail ? (userEmail === 'treasurer@skill.edu' ? 'treasurer' : 'staff') : 'student'))));
